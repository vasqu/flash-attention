...
OUT="${1:-benchmark_results/indexed_common_5h_$(date -u +%Y%m%d_%H%M%S)}"
mkdir -p "$OUT"

# The 420-case matrix retains the proven 3h timing depth. The extra profiles
# and paired crossover cases make the default run a 5h+ H100 qualification.
WARMUP="${WARMUP:-7}"
ROUNDS="${ROUNDS:-12}"
...
args=(
  ...
  --max-iters "$MAX_ITERS"
  --output-dir "$OUT"
  --run-name indexed_common_5h_bf16
)
...
python benchmarks/summarize_indexed_common_3h.py \
  "$OUT/indexed_common_5h_bf16.json" \
  --output "$OUT/INDEXED_COMMON_5H_REPORT.md"

echo "Results: $OUT"
echo "Report:  $OUT/INDEXED_COMMON_5H_REPORT.md"
